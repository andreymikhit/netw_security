...
- [V] ? Можно, при создании цели в разделе Credentials for authenticated checks
* Нельзя
...
- [V] Хранения вспомогательных модулей, не используются для реализации эксплойтов
...
- [V] meterpreter>hashdump. В файле SAM
...
- [V] С помощью модуля post/windows/manage/persistence_exe (создает сценарий автозапуска для выбранного файла)
...
- [V] ? метод оценки безопасности компьютерных систем или сетей средствами моделирования атаки злоумышленника
...
### 10. Какая из ниже приведенных команд позволяет вывести список возможных обновлений и отфильтровать только обновления безопасности?
- [V] sudo apt list --upgradable | grep security
...
- [V] статья 273 ук рф может быть связана с metasploit framework - если не в учебных целях и по договору
- [ ] 
...
- [V] ? Выполнить команду getsystem в meterpreter
...
- [V] ? Extreme Bruteforcer EGB, Elcomsoft EDPR
- [ ] 
* ~  aircrack-ng
...
- [V] AV:N/AC:L/PR:L/UI:N/S:U/C:L/I:N/A:H
...
- [V] Social Engineering Toolkit (SET), BeeF, NMap
* Ettercap
...
- [V] Комплексный процесс, включающий повторяющиеся мероприятия (Мониторинг-->Защита-->Аудит) со стороны оператора связи
...
- [V] POST. Предназначен для запроса, при котором указанный ресурс принимает данные, заключенные в тело сообщения для хранения
...
- [V] ? Мониторинг сети для получения полной информации о проходящем трафике на данном сетевом интерфейсе;
...
- [V] ? воспользоваться поисковиками уязвимостей (Shodan, Censys, Binary Edge, Zoom Eye и т. д.)
...
- [V] ? Root, связано это с тем, что данный тип сканирования требует доступа к raw sockets
...
- [V] aireplay-ng -0 1 -а 00:40:48:7E:4A:6C -с @0:AF:H4:A3:1E:91 wlan0mon
...
- [V] ? Отправлять в сетевую среду провокационные сообщения с заранее предусмотренными IP-адресами, которых в теории нет и не должно быть в DNS-кэше хакера. Это связано с тем, что все снифферы по умолчанию пытаются отрезолвить (PTR) неизвестные им IP-адреса
...
- [V] Поиск уязвимости Ethernalblue с помощью NMap нa IP 192.168.59.4
...
- [V] Для создания файла с предвычисленными вариантами хэшей паролей (радужные таблицы), который используется в целях ускорения атаки подбора паролей WiFi. Утилита genpmk входит в состав Cowpatty
...
- [V] systemctl enable bluetooth.servce
...
- [V] Использовать онлайн-декодеры либо команды декодирования в операционной системе Linux
...
- [V] Параметр lport указывается, чтобы созданный вредоносный файл после запуска на компьютере жертвы подключился к указанному порту нa Kali Linux. Значение порта может быть любым.
...
- [V] Поднять поддельную базовую станцию, работающую в режимах 2G/3G, и реализовать MiTM-атаку
...
* [V] ? В зависимости от используемого устройства, от десятых до сотых долей секунды
* От 30 до 40 минут
* От 2 до 3 часов
...
* [V] ? Нет, только снизить уровень обнаружения, так как MSF использует по большей части шаблоны известные для всех разработчиков антивирусного ПО
...
* [V] ? Да, но спектр более ограниченный
...
* [V] ? Назвать поддельную точку доступа общеизвестным именем, например MT_Free (название точек доступа в Московском метрополитене), что приведет к автоматическому подключенив WiFi-клиентов, находящихся в зоне действия поддельной AP
...
* [ ] ? Через мастер создания сканирования, предварительно создав профиль и заполнив настройки
...
* [ ] ? НLR (домашний регистр) + BCS (контроллер базовых станций) + IMSI (международный идентификатор мобильного абонента)
...
* [V] 70%, отсеивает менее точные результаты по обнаруженным уязвимостям
...
приложений?
* [V] SSLsplit - подмена сертификата TSL/SSL с целью прослушивания трафика
...
* [V] На некоторых WiFi-точках доступа можно включить защитный механизм (Protected Management Frames, PMF) либо мониторинг эфира для выявления принудительной деаутентифакации клиентов беспроводных сетей
...
* [V] Гораздо сложнее осуществлять перехват данных
...
### 55. Как называется атака, при которой злоумышленник тайно ретранслирует и при необходимости изменяет связь между двумя сторонами,
которые считают, что они непосредственно общаются друг с другом?
* [V] Человек посередине (MITM)
...
* [V] пассивное прослушивание, фальшивая базовая станция, клонирование sim-карты
