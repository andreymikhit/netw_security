...
* отв ***airckrack-ng /nedia/shr/wpa.full.cap; airmon-ng start wlan0***
...
* отв ? Можно, при создании цели в разделе Credentials for authenticated checks
* Нельзя
...
* отв ***Хранения вспомогательных модулей, не используются для реализации эксплойтов***
...
* отв ***meterpreter>hashdump. В файле SAM***
...
* отв ***С помощью модуля post/windows/manage/persistence_exe (создает сценарий автозапуска для выбранного файла)***
...
* отв ? метод оценки безопасности компьютерных систем или сетей средствами моделирования атаки злоумышленника
...
безопасности?
* отв ***sudo apt list --upgradable | grep security
...
* отв ***WiFi-honeypot, WiFi IDS/IPS***
* NGFW, UTM
...
* отв ***статья 273 ук рф может быть связана с metasploit framework - если не в учебных целях и по договору***
...
* отв ***Extreme Bruteforcer EGB, Elcomsoft EDPR***
~~aircrack-ng
...
* отв ***AV:N/AC:L/PR:L/UI:N/S:U/C:L/I:N/A:H***
...
* отв ***Social Engineering Toolkit (SET), BeeF, NMap***
* Ettercap
...
* отв ***дейтаграмны с подменой IР-адресов реальных марирутизаторов***
...
* отв ? Комплексный процесс, включающий повторяющиеся мероприятия (Мониторинг-->Защита-->Аудит) со стороны оператора связи
...
* отв ? Мониторинг сети для получения полной информации о проходящем трафике на данном сетевом интерфейсе;
...
* отв ? воспользоваться поисковиками уязвимостей (Shodan, Censys, Binary Edge, Zoom Eye и т. д.)
...
* отв ***aireplay-ng -0 1 -а 00:40:48:7E:4A:6C -с @0:AF:H4:A3:1E:91 wlan0mon***
...
* отв ? Отправлять в сетевую среду провокационные сообщения с заранее предусмотренными IP-адресами, которых в теории нет и не должно быть в DNS-кэше хакера. Это связано с тем, что все снифферы по умолчанию пытаются отрезолвить (PTR) неизвестные им IP-адреса
...
* отв ***Поиск уязвимости Ethernalblue с помощью NMap нa IP 192.168.59.4***
...
* отв ***Для создания файла с предвычисленными вариантами хэшей паролей (радужные таблицы), который используется в целях ускорения атаки подбора паролей WiFi. Утилита genpmk входит в состав Cowpatty***
