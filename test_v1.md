...
* о ***airckrack-ng /nedia/shr/wpa.full.cap; airmon-ng start wlan0**
...
* о ***Хранения вспомогательных модулей, не используются для реализации эксплойтов***
...
* о ***meterpreter>hashdump. В файле SAM***
...
* о ***С помощью модуля post/windows/manage/persistence_exe (создает сценарий автозапуска для выбранного файла)***
...
безопасности?
* о ***sudo apt list --upgradable | grep security
...
* о ***статья 273 ук рф может быть связана с metasploit framework - если не в учебных целях и по договору***
...
16. Какая из перечисленных утилит может использоваться для bruteforce-атаки на hash-паролей на GPU?
* о ***Extreme Bruteforcer EGB, Elcomsoft EDPR***
~~aircrack-ng
...
17. В каких файлах хранятся захешированные пароли и учетные данные в текстовом виде в Linux-системах?
* о ***/etc/shadow и /etc/passwd***
* В файле SAM
* /etc/mater.passwd
* ~~/etc/shadow
...
18. Расшифруйте базовую оценку CVSSv3. В каком из векторов CVSSv3 присутствуют перечисленные метрики: сложность атаки — низкая, вектор атаки — сетевой, влияние на доступность — высокое?
(по условию AC:L AV:N A:H)
* о ***AV:N/AC:L/PR:L/UI:N/S:U/C:L/I:N/A:H***
...
19. С какими из перечисленных утилит можно интегрировать MSF?
* о ***Social Engineering Toolkit (SET), BeeF, NMap***
* Ettercap
...
20. Какую информацию с поддельного маршрутизатора рассылает злоумышленник для того, чтобы провести атаку на протокол RIP?
* о ***дейтаграмны с подменой IР-адресов реальных марирутизаторов***
...
21. В процессе анализа дампа мы обнаружили в ответных пакетах от интересующих нас хостов RST, ACK-флаги. Что они означают?
* о ?
* 
...
22. Какой из приведенных вариантов содержит наиболее подходящие методы защиты для сетей на базе сигнального протокола Diameter, учитывая выявленные в нем слабые места?
...
23. Какой из перечисленных методов HTTP необходимо искать в случае подозрения перехвата логина и пароля в открытом виде при разборе дампа сетевого трафика в WireShark:
* ***POST***
*
*
...
24. Мы хотим запустить наш WiFi-адаптер в режим мониторинга, какую команду будем использовать при работе с пакетом air-?
...
25. С какой целью администратор сети может включать неразборчивый режим (promiscuous mode) на сетевом интерфейсе?
